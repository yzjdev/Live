...
import app.live.droid.UA_MOBILE
...
object DouyuRepository {
    ...
    fun getLives(page: Int) = liveData(Dispatchers.IO) {
        ...
        val result = try {
            ...
            val arr =
                JsonParser().parse(json).asJsonObject.get("data").asJsonObject.get("rl").asJsonArray
            arr.forEach { e ->
                e.asJsonObject.apply {
                    ...
                    val avatar =
                        "https://apic.douyucdn.cn/upload/" + get("av").asString + "_middle.jpg"
