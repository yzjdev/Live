...
object KuaishouRepository {
    ...
    fun getLives(pageNo: Int) = liveData(Dispatchers.IO) {
        ...
        val result = try {
            val json = KuaishouNetwork.getLives(pageNo)
            val arr =
...
class KuaiShou {
    fun getRealUrl(room: String): StreamBean{
        val res = HttpRequest.get("https://live.kuaishou.com/u/$room").header("User-Agent", Config.UA_PC).execute().body()
                    ...
                    getJSONObject(0)
                        ...
                        .forEach { item ->
                            item.toJSONString().parseObject().apply {
                                urls.add(URLDecoder.decode(getString("url"), CharsetUtil.CHARSET_UTF_8))
